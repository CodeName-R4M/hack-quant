...
def test_baseline_controllers_comparison():
    """Runs Fixed and Rule-Based controllers on identical seeded scenario and prints comparison table."""
    seed = 42
    duration = 240
    network = RoadNetwork(DEFAULT_CONFIG.network)
    ...
    df = pd.DataFrame([res_fixed, res_rule])
    print("\n--- PHASE 2: BASELINE CONTROLLERS BENCHMARK TABLE ---")
    ...
    assert res_rule["avg_wait_sec"] > 0
